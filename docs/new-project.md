...
**Folder + GitHub:** use an **all-lowercase** `repo-slug` (letters, digits, hyphens only—no mixed case). The workspace directory is **`$SFS_WORKSPACE_ROOT/<repo-slug>`** (default **`~/Documents/CURSOR/<repo-slug>`**). Example: slug **`sfsagency`** → **`~/Documents/CURSOR/sfsagency`** and remote **`cfolsensfs/sfsagency`**. Keeping local folder and slug lowercase avoids “two names” confusion.
...
Tag releases on this repo so consumers can reference `**manifest.yaml**` and a known-good version. **New projects:** pin **`v1.1.0`** or newer. **v1.0.0** remains the first public capture baseline. Release checklist: [releasing.md](releasing.md).