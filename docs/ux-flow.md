...
| **Distill rhythm** | Use recent **`local/chat-log/*.md`** with **inbox stubs** ([automation.md](automation.md)) and **git diff** so “why” in chat can land in **`.calyx/reasoning/`** without retyping. For **org lift** and taxonomy, follow **`.calyx/AGENT_ROLES.md`** and **`org-lift-cadence.txt`** after **`calyx-install-agent-roles.sh`**. |
